...
const Library = () => {
  const { selectedAlbum, songs, setSelectedSong, playingIndex, setPlayingIndex, setIsPlaying } = useSongContext();
  ...
  return (
    <div className="bg-[#121212] w-full h-full overflow-y-scroll p-2.5">
      <ul className="space-y-1">
        {songs && songs.length > 0 ? (
          songs.map((song, index) => (
            <li
              key={index}
              className={`flex items-center justify-between bg-transparent border-2 border-[#2a2929] px-3.5 py-2 rounded-lg hover:bg-[#3a3939] cursor-pointer transition-all duration-200 ${
                playingIndex === index ? "bg-[#2d2b2b]" : ""
              }`}
            >
              <div className="flex items-center gap-2" onClick={() => handlePlayClick(song, index)}> 
              ...
              >
                <h3 className="text-[.8rem] font-semibold text-white">
                  {playingIndex === index ? "Pause" : "Play Now"}
                </h3>
                {playingIndex === index ? (
      ...
      <p className="text-gray-200 text-sm font-bold px-3.5 flex items-center gap-1.5">
        <GiMusicalNotes className="size-4.5" /> No songs available...
      </p>
        )}
    </ul>
    </div >
